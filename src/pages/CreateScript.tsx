...
const scriptTypes = [
  ...
  { value: 'shorts_script', label: 'Shorts / Reels Script' },
  { value: 'podcast_script', label: 'Podcast Script' },
  { value: 'documentary_script', label: 'Documentary Script' },
  { value: 'movie_scene_script', label: 'Movie Scene Script' },
  { value: 'commercial_script', label: 'Commercial Script' },
  { value: 'voiceover_narration', label: 'Voice-over Narration' },
];
...
const formatTypes = [
  { value: 'script', label: 'Script (scene-by-scene flow)' },
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(errorData.error || 'Failed to generate script');
      ...
      await supabase.from('content_history').insert({
        ...
        content: fullContent,
        content_type: scriptType as any,
        tone: tone as any,
        topic: topic.trim(),
            ...
            <div className="bg-card border border-border rounded-xl sm:rounded-2xl p-4 sm:p-6 space-y-4 sm:space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Select value={formatType} onValueChange={setFormatType}>
                  <SelectTrigger className="h-10 sm:h-11">
                    <SelectValue placeholder="Auto (based on script type)" />
                  </SelectTrigger>
                  <SelectContent>
                    {formatTypes.map((f) => (
